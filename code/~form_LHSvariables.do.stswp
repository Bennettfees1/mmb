...
//egen rrate_cum${horizon} = sum(rrate) if period<=$horizon 	& rrate_chg_sign_sum<1, by(id)
//egen pi_cum${horizon} = sum(pi) if period<=$horizon 			& pi_chg_sign_sum<1, by(id)

egen piq_cum${horizon} = sum(piq) if period<=$horizon 		, by(id)
egen y_cum${horizon} = sum(y) if period<=$horizon 			, by(id)
egen irate_cum${horizon} = sum(irate) if period<=$horizon 	, by(id)
egen rrate_cum${horizon} = sum(rrate) if period<=$horizon 	, by(id)
egen pi_cum${horizon} = sum(pi) if period<=$horizon 		, by(id)
